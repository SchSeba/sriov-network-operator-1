...
var _ = Describe("Drainer", Ordered, func() {
	...
	Context("DrainNode", func() {
		It("should failed if the platform is not able to drain", func() {
			...
			completed, err := drn.DrainNode(ctx, n, false)
		...
		It("should return error if not able to cordon the node", func() {
			...
			_, err := drn.DrainNode(ctx, nCopy, false)
			Expect(err).To(HaveOccurred())
		})
		...
		It("should return error if not able to drain the node", func() {
			...
			_, err := drn.DrainNode(ctx, n, true)
			Expect(err).To(HaveOccurred())
		})
		...
		It("should remove pods with sriov devices only if a full drain is not requested", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			platformHelper.EXPECT().OpenshiftBeforeDrainNode(ctx, n).Return(true, nil)
			createPodOnNode(ctx, "test-node-1", "node1")
			createPodWithSriovDeviceOnNode(ctx, "test1-node-1", "node1")

			go func() {
				Eventually(func(g Gomega) {
					podObj := &corev1.Pod{}
					err = k8sClient.Get(ctx, client.ObjectKey{Name: "test1-node-1", Namespace: testNamespace}, podObj)
			...
			_, err = drn.DrainNode(ctx, n, false)
			Expect(err).ToNot(HaveOccurred())
			pod := &corev1.Pod{}
			err = k8sClient.Get(ctx, client.ObjectKey{Name: "test-node-1", Namespace: testNamespace}, pod)
			Expect(err).ToNot(HaveOccurred())
			err = k8sClient.Get(ctx, client.ObjectKey{Name: "test1-node-1", Namespace: testNamespace}, pod)
			Expect(err).To(HaveOccurred())
