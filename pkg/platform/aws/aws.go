...
func (a *Aws) DiscoverSriovDevices() ([]sriovnetworkv1.InterfaceExt, error) {
	...
	for _, iface := range a.loadedDevicesInfo {
		driver, err := dputils.GetDriverName(iface.PciAddress)
		if err != nil {
			funcLog.Error(err, "unable to parse device driver for device, skipping",
				"iface", iface)
			continue
		}
		iface.Driver = driver
...
func (a *Aws) createDevicesInfo() error {
	...
	for _, macEntry := range macEntries {
		...
		macAddressToSubNetID[macEntry] = subnetIDData
	}
